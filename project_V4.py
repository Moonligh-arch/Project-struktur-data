    ...
    def tampilkan_status_lengkap(self):
        ...
        if not self.penumpang_dalam:
            ...
            for p in self.penumpang_dalam:
                teks = f"• {p.nama} → Tujuan Lt.{p.tujuan}"
                print(f"  │  {teks:<39}│")
        print("  └─────────────────────────────────────────┘")

        print("  ┌── Antrean Panggilan Luar ───────────────┐")
        ada_antrean = False
        for lantai in range(lantai_max, lantai_min - 1, -1):
            naik = self.panggilan_luar[lantai]["naik"]
            turun = self.panggilan_luar[lantai]["turun"]
            if naik or turun:
                ada_antrean = True
                print(f"  │  Lantai {lantai:<37}│")
                if naik:
                    teks_naik = ", ".join([f"{p.nama}(ke L{p.tujuan})" for p in naik])
                    if len(teks_naik) > 26:
                        teks_naik = teks_naik[:23] + "..."
                    print(f"  │     [NAIK]  {teks_naik:<26}│")
                if turun:
                    teks_turun = ", ".join([f"{p.nama}(ke L{p.tujuan})" for p in turun])
...
def main():
    ...
    while True:
        ...
        print(" 3. Lihat Status Lift & Antrean")
